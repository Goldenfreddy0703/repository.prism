...
class ContextAction(metaclass=ABCMeta):
    ...
    def log_action(self, simkl_id):
        log(f"{self.name} ({simkl_id})")

    def log_error(self):
        log(f"Missing required simkl_id from arguments ({self.action_args})", xbmc.LOGERROR)
    ...
    def execute(self):
        if simkl_id := self.action_args.get("simkl_id"):
            self.handle_args(simkl_id=simkl_id)
            self._pop_keys()
            self.handle_path()
            self.log_action(simkl_id)
