...
def anime_source_from_list_context() -> bool:
    """True when the active list was opened from an anime Prism menu."""
    folder = (xbmc.getInfoLabel("ListItem.FolderPath") or "").lower()
    container = (xbmc.getInfoLabel("Container.FolderPath") or "").lower()
    combined = f"{folder} {container}"
    if "catalog=anime" in combined:
        return True
    if "list_id=anime_" in combined:
        return True
    anime_actions = (
        "discoveranime",
        "myanime",
        "ondeckanime",
        "animenextup",
        "animerecentlywatched",
        "animewatchedepisodes",
        "simkllibrarylist",
    )
    if any(action in combined for action in anime_actions):
        return True
    return "animeshows" in combined or "animemovies" in combined


def attach_source_catalog(action_args: dict) -> dict:
    """Stamp catalog=anime on action_args when opened from an anime list."""
    if not isinstance(action_args, dict):
        return action_args
    if anime_source_from_list_context():
        action_args = dict(action_args)
        action_args["catalog"] = "anime"
    return action_args
        ...
        if action_arg_str := query_parameters.get("action_args"):
            with contextlib.suppress(json.JSONDecodeError):
                ...
                if action_args and isinstance(action_args, dict):
                    return normalize_action_args(action_args)
    return {}
