"""
TorBox debrid service integration for Prism.
...
class TorBox:
    ...
    def auth(self):
        """
        Authenticate with TorBox using the OAuth device-code flow.

        Mirrors the device-code auth used by Real-Debrid / AllDebrid / Premiumize:
        ...
        verification_url = (
            resp.get("friendly_verification_url")
    ...
    def _request_device_code(self):
        """Start the TorBox device-code flow (no token required)."""
        try:
            response = self.session.get(
                self.base_url + "user/auth/device/start",
                params={"app": g.ADDON_NAME},
                headers={"User-Agent": f"Prism/{g.VERSION}"},
                timeout=20,
            )
        except Exception as e:
            g.log(f"TorBox auth start error: {e}", "error")
            return None

        if not response.ok:
            g.log(f"TorBox auth start returned {response.status_code}: {response.text}", "error")
            return None
    ...
    @staticmethod
    def _device_code_ttl(expires_at, default=600):
        """Seconds until the device code expires, derived from the expires_at timestamp."""
        import datetime
        ...
        try:
            ...
            return max(int((expires_dt - now).total_seconds()), 60)
        except (ValueError, TypeError):
            return default

    def _auth_loop(self, device_code):
        """Poll the TorBox device token endpoint; returns True once a token is issued."""
        try:
            response = self.session.post(
                self.base_url + "user/auth/device/token",
                json={"device_code": device_code},
                headers={"User-Agent": f"Prism/{g.VERSION}", "Content-Type": "application/json"},
                timeout=20,
            )
        except Exception as e:
            g.log(f"TorBox auth poll error: {e}", "error")
            return False
    ...
    # TorBox plan tier -> display label (see TorBox /user/me)
    _PLAN_LABELS = {0: "Free", 1: "Essential", 2: "Pro", 3: "Standard"}
    ...
    def _save_user_status(self):
        ...
        if account_info:
            ...
            plan = account_info.get("plan", 0) or 0
            status = self._PLAN_LABELS.get(plan, "Premium" if plan > 0 else "Free")
