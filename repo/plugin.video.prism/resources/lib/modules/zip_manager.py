...
class ZipManager:
    ...
    def _extract_zip_member(self, member, output_path):
        # Strip only the LEADING root-directory prefix. Using str.replace here would
        # remove every occurrence, which flattens packages whose zip root name matches
        # an inner folder name (e.g. a4kScrapers/providers/a4kScrapers/... -> providers/...).
        if self._root_directory and member.startswith(self._root_directory):
            relative_member = member[len(self._root_directory):]
        else:
            relative_member = member
        target_path = os.path.join(output_path, relative_member)
