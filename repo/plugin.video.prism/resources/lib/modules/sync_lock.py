...
class SyncLock:
    def __init__(self, lock_name, simkl_ids=None):
        self._lock_name = lock_name
        self._lock_format = "{}.SyncLock.{}.{}"
        self._simkl_ids = simkl_ids or []
    ...
    def _run(self):
        self._running_ids = {
            self._run_id(i) for i in self._simkl_ids if not g.get_bool_runtime_setting(self._create_key(i))
        }

    def _still_processing(self):
        return any(g.get_bool_runtime_setting(self._create_key(i)) for i in self._simkl_ids)
