...
class Cache(CacheBase):
    ...
    def __init__(self):
        ...
        self._auto_clean_interval = datetime.timedelta(hours=2)
    ...
    def trim_disk_rows(self, max_rows: int) -> int:
        return self._db_cache.trim_disk_rows(max_rows)

    def purge_disk_older_than(self, cutoff_timestamp: float) -> int:
        return self._db_cache.purge_older_than(cutoff_timestamp)
...
class DatabaseCache(Database, CacheBase):
    ...
    def trim_disk_rows(self, max_rows: int) -> int:
        if max_rows <= 0:
            return 0
        count_row = self.fetchone(f"SELECT COUNT(*) AS cnt FROM {self.cache_table_name}")
        total = int(count_row["cnt"]) if count_row and count_row.get("cnt") is not None else 0
        if total <= max_rows:
            return 0
        to_remove = total - max_rows
        self.execute_sql(
            f"""
            DELETE FROM {self.cache_table_name}
            WHERE id IN (
                SELECT id FROM {self.cache_table_name}
    ...
    def purge_older_than(self, cutoff_timestamp: float) -> int:
        before = self.fetchone(f"SELECT COUNT(*) AS cnt FROM {self.cache_table_name}")
        self.execute_sql(
            f"DELETE FROM {self.cache_table_name} WHERE expires < ?",
            (float(cutoff_timestamp),),
        )
        after = self.fetchone(f"SELECT COUNT(*) AS cnt FROM {self.cache_table_name}")
        before_count = int(before["cnt"]) if before and before.get("cnt") is not None else 0
        after_count = int(after["cnt"]) if after and after.get("cnt") is not None else 0
...
def use_cache(cache_hours=8):
    ...
    def _get_checksum(class_name, func_name):
        relative_methods = {"SimklAPI": {"get_json_cached": ("item.limit",)}}
    ...
    def _cache_scope(func, args):
        """Build cache namespace + positional args slice for methods vs module functions."""
        params = list(inspect.signature(func).parameters.keys())
        if params and params[0] in ("self", "cls") and args:
            owner = args[0]
            scope = owner.__class__.__name__
            cache_args = args[1:]
        else:
            scope = func.__qualname__
            cache_args = args
        return scope, cache_args

    def _decorator(func):
        @wraps(func)
        def _decorated(*args, **kwargs):
            cache_scope, cache_args = _cache_scope(func, args)
            ...
            checksum = _get_checksum(cache_scope, func.__name__)
            cache_str = "{}.{}.{}.{}".format(
                cache_scope,
                func.__name__,
                tools.md5_hash(cache_args),
